...
# using the mimosa image simply because it has everything installed we need to do all preprocessing
...
git -C preprocessed/sub-$sub checkout -b "sub-$sub"

export SINGULARITYENV_TMPDIR=$TMPDIR
t1=$(find $PWD/fmriprep/sub-${sub} -name "sub-${sub}_desc-preproc_T1w.nii.gz" | head -n1)
t1_mask=$(find $PWD/fmriprep/sub-${sub} -name "sub-${sub}_desc-brain_mask.nii.gz" | head -n1)
flair=$(find $PWD/nifti/sub-${sub} -name "sub-${sub}_ses-01_run-001_FLAIR.nii.gz" | head -n1)
outdir=$PWD/preprocessed/sub-${sub}/anat
mkdir -p $outdir
datalad run -i "nifti/sub-${sub}" -i "fmriprep/sub-${sub}" -i "simg/mimosa_latest.sif" -o "preprocessed/sub-${sub}" -m "registered $t1 to $flair" \
    singularity exec --cleanenv \
    -B /project -B $TMPDIR \
    $PWD/simg/mimosa_latest.sif \
    Rscript ./code/preprocessing.R $t1 $t1_mask $flair $outdir

# selectively push outputs only
# ignore root dataset, despite recorded changes, needs coordinated
# merge at receiving end
flock $DSLOCKFILE datalad push -d preprocessed/sub-${sub} --to origin
