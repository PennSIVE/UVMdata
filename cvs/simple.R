...
setwd("/cbica/projects/UVMdata/dropbox/repro7again_jordan_new")
files=list.files()
...
for(i in files){
  ...
  writenii(out$cvs.probmap,paste0("/cbica/projects/UVMdata/dropbox/repro7again_jordan_new_iter_n/",i,".unclean.cvs.probmap.nii.gz"))
  newdf=rbind(newdf,data.frame(id=i,biomarker=out$cvs.biomarker))
  print(c(i,out$cvs.biomarker))
  write.csv(newdf,"/cbica/projects/UVMdata/dropbox/repro7again_jordan_new_iter_n/unclean_results.csv")
}
