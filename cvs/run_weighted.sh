...
do
    ...
    SINGULARITYENV_NSLOTS=\$NSLOTS singularity run --cleanenv \
    -B ~/repos/UVMdata/cvs:/code:ro \
    -B ~/dropbox/jordan_new/${subj}/candidate.lesions.nii.gz:/lesmap.nii.gz:ro \
    -B ~/dropbox/jordan_new/${subj}/frangi.nii.gz:/veinmap.nii.gz:ro \
    -B ~/dropbox/jordan_new/${subj}/epi_brain.nii.gz:/epi.nii.gz:ro \
