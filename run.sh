...
wd="/cbica/projects/UVMdata/repos/UVMdata"
...
splice_jid=$(qsub -terse -hold_jid $zscore_jid -o splice.log -e splce.err -cwd $wd/shell/splice_gifti_metadata.sh)

# regenerate all CIFTIs from GIFTIs
cd /cbica/projects/UVMdata/analysis
ls | grep ^sub- | tr "-" "\n" | grep -v sub | xargs -I {} \
qsub -hold_jid $splice_jid -b y -cwd -l short singularity exec -B $PWD/sub-{}/MNINonLinear:/input -B $wd/shell/gifti2cifti.sh:/gifti2cifti.sh /cbica/home/robertft/singularity_images/hcppipelines_latest.sif /gifti2cifti.sh /input {}
